import React from 'react';
import { ArrowLeft } from 'lucide-react';
import { motion } from 'framer-motion';

interface RegistrationPageProps {
  onBack: () => void;
}

export function RegistrationPage({ onBack }: RegistrationPageProps) {
  const handleGoogleLogin = (type: 'paid' | 'waitlist') => {
    // Auth logic placeholder
    console.log(`Registering with Google (${type})...`);
  };

  return (
    <div className="min-h-screen flex items-center justify-center overflow-hidden p-4 bg-zinc-50 dark:bg-black relative">
      {/* Back Button */}
      <button 
        onClick={onBack}
        className="absolute top-4 left-4 z-50 flex items-center gap-2 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 transition-colors bg-white/80 dark:bg-black/80 backdrop-blur-md px-4 py-2 rounded-full border border-zinc-200 dark:border-zinc-800"
      >
        <ArrowLeft size={16} />
        <span className="text-sm font-medium">Back</span>
      </button>

      <div className="w-full relative max-w-7xl overflow-hidden flex flex-col md:flex-row shadow-2xl rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 min-h-[600px]">
        
        {/* Left Box - Paid Early Access */}
        <div className="w-full md:w-1/2 bg-black relative p-8 md:p-12 flex flex-col justify-center items-center text-center overflow-hidden border-b md:border-b-0 md:border-r border-zinc-800">
             <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-zinc-800 via-zinc-950 to-black opacity-50" />
             <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff05_1px,transparent_1px),linear-gradient(to_bottom,#ffffff05_1px,transparent_1px)] bg-[size:32px_32px] opacity-20"></div>
             
             <motion.div 
               initial={{ opacity: 0, y: 20 }}
               animate={{ opacity: 1, y: 0 }}
               transition={{ delay: 0.2 }}
               className="relative z-10 flex flex-col items-center gap-8 max-w-md w-full"
             >
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white leading-tight">
                  Be one of the first users of Hemlo
                </h1>
                
                <div className="flex flex-col items-center gap-1 p-4 rounded-xl bg-white/5 border border-white/10 w-full">
                    <span className="text-zinc-400 text-sm font-medium tracking-wide uppercase">Early Access Pass</span>
                    <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-bold text-white">$5</span>
                        <span className="text-sm text-zinc-500 font-medium">/ one-time</span>
                    </div>
                </div>

                <button 
                  onClick={() => handleGoogleLogin('paid')}
                  className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-200 text-black py-4 px-6 rounded-xl font-bold transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5 active:scale-[0.98] group"
                >
                   <svg className="w-6 h-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                    </svg>
                    <span>Pay & Join with Google</span>
                </button>
             </motion.div>
        </div>
 
        {/* Right Box - Free Waitlist */}
        <div className="w-full md:w-1/2 bg-white dark:bg-zinc-900 p-8 md:p-12 flex flex-col justify-center items-center text-center relative">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-orange-100/40 via-transparent to-transparent dark:from-orange-900/10" />
            
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 0.4 }}
              className="relative z-10 flex flex-col items-center gap-8 max-w-md w-full"
            >
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-zinc-900 dark:text-white leading-tight">
                  Notify me when available worldwide
                </h2>

                <div className="flex flex-col items-center gap-1 p-4 rounded-xl bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-800 w-full">
                    <span className="text-zinc-500 font-medium tracking-wide uppercase text-sm">Global Launch List</span>
                    <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-bold text-zinc-900 dark:text-white">Free</span>
                    </div>
                </div>

                <button 
                  onClick={() => handleGoogleLogin('waitlist')}
                  className="w-full flex items-center justify-center gap-3 bg-zinc-900 hover:bg-zinc-800 dark:bg-white dark:hover:bg-zinc-200 text-white dark:text-black py-4 px-6 rounded-xl font-bold transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5 active:scale-[0.98] group"
                >
                   <svg className="w-6 h-6 group-hover:scale-110 transition-transform duration-300" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                    </svg>
                    <span>Join Waitlist with Google</span>
                </button>
            </motion.div>
        </div>
      </div>
    </div>
  );
}