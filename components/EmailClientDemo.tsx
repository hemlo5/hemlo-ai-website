             ...
             <AnimatePresence mode="wait">
                 {(step === 0 || step === 1) && (
                     <motion.div 
                        ...
                        className="w-full max-w-[340px] bg-zinc-900/90 border border-zinc-800 backdrop-blur-sm rounded-xl shadow-2xl overflow-hidden"
                     >
                        {/* Header */}
                        <div className="px-4 py-3 border-b border-zinc-800 flex items-center gap-3 bg-zinc-900/50">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-zinc-300 truncate font-medium">Subject: Q3 Performance Review</div>
                            </div>
                            <Paperclip size={14} className="text-zinc-600" />
                        </div>
                        
                        {/* Body */}
                        <div className="p-5 h-40 font-mono text-sm text-zinc-300 leading-relaxed overflow-hidden relative">
                             {step === 0 && (
                        ...
                        {/* Footer / Send Button */}
                        <div className="px-4 py-3 bg-zinc-900 border-t border-zinc-800 flex justify-end">
                            <div className="flex items-center gap-2 px-4 py-1.5 bg-white rounded-full text-black text-xs font-bold shadow-lg shadow-white/10">
                                <span>Send</span>
                 ...
                 {step === 2 && (
                     <motion.div
                        ...
                        className="w-full max-w-[340px] bg-zinc-900/90 border border-zinc-800 backdrop-blur-sm rounded-xl shadow-2xl overflow-hidden absolute"
                     >
                        {/* Static visual of the card for smooth exit transition */}
                        <div className="px-4 py-3 border-b border-zinc-800 flex items-center gap-3 bg-zinc-900/50">
                            <div className="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center text-indigo-400 text-xs font-bold border border-indigo-500/30">AI</div>
                            <div className="flex-1 min-w-0">
                                <div className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold mb-0.5">Generating Draft</div>
                                <div className="text-xs text-zinc-300 truncate font-medium">Subject: Q3 Performance Review</div>
                            </div>
                        ...
                        <div className="p-5 h-40 font-mono text-sm text-zinc-100 leading-relaxed">
                            Hi Team, I've analyzed the Q3 data. Revenue is up 15% YoY. I've attached the full report for tomorrow's sync.
                        </div>
                        <div className="px-4 py-3 bg-zinc-900 border-t border-zinc-800 flex justify-end">
                            <div className="flex items-center gap-2 px-4 py-1.5 bg-white rounded-full text-black text-xs font-bold shadow-lg">
                                <span>Send</span>
                                <Send size={12} />
                            </div>
                        </div>
                 ...
                 {step === 3 && (
                    <motion.div
                        ...
                        className="absolute flex flex-col items-center gap-3 bg-zinc-900/80 p-6 rounded-2xl border border-zinc-800 backdrop-blur-sm"
                    >
                        <div className="w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center border border-emerald-500/50 text-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                            <Check size={24} strokeWidth={3} />
                        </div>
                        <div className="text-white font-semibold text-sm">Sent Successfully</div>
                    </motion.div>
                 )}
             </AnimatePresence>
        </div>
